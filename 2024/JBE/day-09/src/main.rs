...
fn part_1(line: &Line) -> isize {
    ...
    for (idx, &(file_id, file_len, mut free_len)) in line.files.iter().enumerate() {
        ...
        if idx == consuming_file_idx {
            ...
            sum += score(position, file_id, length);
            break;
        }
        // add file hashsum
        sum += score(position, file_id, file_len);
        position += file_len;
        ...
        while free_len > 0 {
            ...
            if consuming_remaining_len <= free_len {
                sum += score(position, file_id, consuming_remaining_len);
                position += consuming_remaining_len;
                free_len -= consuming_remaining_len;
                consumed_file_len = 0;
                consuming_file_idx -= 1;
                if consuming_file_idx == idx {
                    break;
                }
            } else {
                // else, add only part of the buffer and update pointers
                consumed_file_len += free_len;
                sum += score(position, file_id, free_len);
                position += free_len;
                free_len = 0;
            }
        }
